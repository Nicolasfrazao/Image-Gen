...
export default function Home() {
  ...
  // This is a custom hook that calls the provided function every X milliseconds
  // until the function returns a value that is truthy
  useInterval(
    async () => {
      // Make a GET request to the /api/poll endpoint with the messageId as a query parameter
      const res = await fetch(`/api/poll?id=${messageId}`);
      const json = await res.json();

      // If the response is successful, set the image state variable to the first item in the response data
      if (res.status === 200) {
        setLoading(false);
        setImage(json.data[0].url);
      }
    },
    // If the loading state variable is true, call the function every 1000 milliseconds
    loading ? 1000 : null
  );
  ...
  // This is a boolean that is true if the loading state variable is true
  // or if the image state variable is truthy and the canShowImage state variable is false
      ...
      <div className="antialiased mx-auto px-4 py-20 h-screen bg-gray-100">
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          >
            ...
            >
              {showLoadingState && (
                // This is a loading spinner that is displayed when the loading state variable is true
                <svg
